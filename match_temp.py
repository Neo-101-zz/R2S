...
if __name__ == '__main__':
    ...
    for year in ascat_year_list:
        ...
        ndbc_files.sort()
        dir = base_dir + 'ascat/pickle/' + year
        ...
        for ndbc_file in ndbc_files:
            ...
            for file in files:
                month = int(file[6:-4])
                pickle_ascat = open(dir + '/' + file, 'rb')
                ascat_data = pickle.load(pickle_ascat)
                print('start to match. '+id+' and month: '+str(month))
                match_list = match(year, month, ascat_data, ndbc_data, 0.25/2, 5)

                pickle_file = open(base_dir+'match/ascat/'+id+'_'+year+'_'+str(month)+'.pkl', 'wb')
                pickle.dump(match_list, pickle_file)
                pickle_file.close()

    qscat_year_list = ['1999', '2000', '2001', '2002']
