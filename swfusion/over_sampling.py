...
def cal_dist_matrix_element(dist_matrix, n, i, data_num, data_nom,
                            feat_count_num, feat_count_nom,
                            feat_ranges_num):
    for j in range(n):
        # breakpoint()
        # if dist_matrix[j][i] != 0.0 and dist_matrix[i][j] == 0.0:
...
def over_sampling(
    ...
    for j in range(d):
        if (data.iloc[:, j].dtype in num_dtypes
            and any(data.iloc[:, j] > 0)):
    ...
    # # Triangle mapping to save half time
    # processes = []
    # for i in tqdm(range(n), ascii = True, desc = "dist_matrix"):
    #     p = mp.Process(target=cal_dist_matrix_element,
    #                    args=(dist_matrix, n, i, data_num,
    #                             data_nom, feat_count_num,
    #                             feat_count_nom, feat_ranges_num))
    #     processes.append(p)
    #     p.start()
    #     # cal_dist_matrix_element(dist_matrix, n, i, data_num,
    #     #                         data_nom, feat_count_num,
    #     #                         feat_count_nom, feat_ranges_num)
        ...
        for i in tqdm(range(n), ascii = True, desc = "synth_matrix"):
            ...
            for j in range(x_synth):
                ...
                if neigh in safe_list:
                    ...
                    ## randomly assign nominal / categorical features
                    ## from observed cases and selected neighbors
                                ...
                                if len(data.iloc[:, x].unique() == 1):
                                    ...
                                    for z in range(len(
                                        ...
                                        probs[z] = len(np.where(
                                            data.iloc[:, x] == data.iloc[
                                                :, x][z]))
                                    
                                    synth_matrix[index_gaus, x] = \
                                            rd.choices(
                                                population = data.iloc[
                                                    :, x].unique(),
                                                weights = probs, 
                                                k = 1)
